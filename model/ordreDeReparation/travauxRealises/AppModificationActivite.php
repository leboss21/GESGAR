...
class AppModificationActivite
{
    ...
    public function ModificationActivite($colonne=array())
    {
        $idTech = $this->ActiteSelecteIdTech($colonne[2]);
        $idRecept = $this->ActiteSelecteIdRecept($colonne[3]);

        $requete = $this->dbaseprivateLite->prepare("UPDATE programmation_activite SET date_activite=:date_activite,heure_debut_activite=:heure_debut_activite,heure_fin_activite=:heure_fin_activite,tache=:tache,id_technicien=:id_technicien,id_reception=:id_reception WHERE id_activite=:id_activite");
        ...
        $requete->bindParam(':id_activite', $colonne[6]);
        if($requete->execute())
        {
            return "oui";
        }
