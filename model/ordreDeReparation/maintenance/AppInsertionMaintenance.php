...
class AppInsertionMaintenance
{
    ...
    public function MaintenanceInsert($colonne=array()){

        $id_reception = $this->MaintSelectIdReception($colonne[4]);
        $id_tech = $this->MaintSelectIdTech($colonne[5]);

        $requete = $this->dbaseprivateLite->prepare("INSERT into maintenance (maintenance_kilometrage,num_ordre,maintenance_date,maintenance_realise,id_reception,id_technicien) VALUES (:maintenance_kilometrage,:num_ordre,:maintenance_date,:maintenance_realise,:id_reception,:id_technicien)");
