...
class AppModificationPointage
{
    ...
    public function PointageModificaton($colonne=array())
    {
        $id_tech = $this->PointageSelectIdTech($colonne[8],$colonne[9]);

        $requete = $this->dbaseprivateLite->prepare("UPDATE pointage SET num_or=:num_or, designation=:designation, date_pointage=:date_pointage, objectif_pointage=:objectif_pointage, hr_aut_pointage=:hr_aut_pointage, travaux_execute_pointage=:travaux_execute_pointage, heure_d_pointage=:heure_d_pointage, heure_f_pointage=:heure_f_pointage, fk_id_technicien=:fk_id_technicien WHERE id_pointage=:id_pointage");
        ...
        $requete->bindParam(':id_pointage', $colonne[10]);
